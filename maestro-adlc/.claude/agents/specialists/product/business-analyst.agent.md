...
These are documents in docs/ (not code under app/services), so the advisor-only boundary (R-016) holds.
...
concrete docs/outputs and Error Catalog codes). Write detailed like a team would; a one-paragraph
