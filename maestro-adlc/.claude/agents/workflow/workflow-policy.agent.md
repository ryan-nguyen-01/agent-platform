...
Also validate migrated workspaces and artifact-only snapshots where application source under `app/` is not available.
...
4. For snapshot checks, detect whether app/ exists. If not, set services_available=false and validate recorded artifacts only.
...
Do not treat missing app/ as a defect when checking an artifact-only snapshot.
```
