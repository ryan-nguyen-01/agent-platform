...
```text
/onboard
  Initial fetch for the current repository. Scans intake/ AND source code.
...
/onboard --refresh <component-id-or-path>
  ...
  Does not rescan intake/.

/onboard --refresh inputs
  Partial refresh of user-provided reference docs only.
  Equivalent to /sync-memory --scan --inputs. Does not touch source/.
...
/onboard --files <paths> [--components <component-ids>]
  ...
  --components optional when all paths are under intake/.
```
...
Added a PRD/HLD/spec file in intake/  | /sync-memory --files intake/<path>
