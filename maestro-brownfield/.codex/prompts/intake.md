---
description: "maestro /intake — Triage whatever the user dropped into the intake folders (`intake/`) so messy input never sile..."
...
Triage whatever the user dropped into the intake folders (`intake/`) so messy
...
```text
1. Scan intake folders (intake/) recursively. For each item classify:
...
2. Write the index (non-destructive — files are NEVER moved/renamed/edited without approval):
   intake/INDEX.md           human view: file -> type -> related component -> status -> flags
   .maestro/registry/intake.yaml   machine view for onboarding/task-analysis to consume
3. FLAGS (each requires a user decision, never silent handling):
   ...
   - misplaced-source: looks like application source dropped into intake/.
     -> ask: "this looks like code for <X> — should it live in source/<X>? move or keep?"
...
📁 Index: intake/INDEX.md + .maestro/registry/intake.yaml
