...
R-002-04: Onboarding must detect repository type, source/modules, stack, dependencies, APIs, DB, CI, environment hints, and test policy.
...
R-002-09: Onboarding must scan intake/ (user-provided reference docs) in addition to source code. Every fact extracted from intake/ must cite source: "intake/<relative-path>".
R-002-10: Onboarding must write .maestro/registry/intake.yaml listing every file under intake/ with path, category, summary, mtime, and confidence.
R-002-11: When intake/ and source code conflict on a fact, code wins for technical/current state; intake/ wins for intent/target state. Conflict must be recorded in project.yaml.conflicts[] with both sources.
R-002-12: Onboarding must not commit, modify, move, or delete files under intake/. It is read-only there.
