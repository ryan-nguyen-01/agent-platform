...
The validator is agent-native and must not require Python, Node, jq, shell scripts, or any local runtime. It is allowed to pass with `services_available: false`; this means `source/` is absent or empty and the check used recorded artifacts only. Missing `source/` is not itself a defect for shared evidence snapshots.
