...
| **coder-data** | **DATA**/LLD (schema) · BA business rules · entity definitions · ERR (so seeded error-path data is valid) | coder-database · business-analyst · `intake/domain/` |
| **coder-database** | **DATA model / entities** (domain glossary, LLD/HLD) · migration policy · NFR (volume) | solution-architect / database-architect · `docs/architecture/`, `intake/domain/` |
...
missing:
  - doc: "API contract"
    ...
    produce_with: "api-designer / solution-architect, or provide docs/architecture/low-level-design or intake/api"
  - doc: "data model (LLD)"
