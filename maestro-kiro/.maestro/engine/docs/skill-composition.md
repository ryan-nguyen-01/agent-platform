...
Runtime agents must not scan or read all of `.kiro/skills/**`. Start with `.maestro/registry/skills.yaml`, select the smallest useful set, then open only the chosen skill's `SKILL.md` and directly referenced files.
