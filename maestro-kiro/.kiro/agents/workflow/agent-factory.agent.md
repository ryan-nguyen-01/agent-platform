...
> Generated service coders are written to `.kiro/agents/coders/` (same place as built-in coders). Workflow agents live in `.kiro/agents/workflow/`; specialist advisors in `.kiro/agents/specialists/<category>/`.
...
5. Write the resolved paths into both:
   - the generated coder agent at .kiro/agents/coders/coder-<service-slug>.agent.md (allowed_write_paths section)
